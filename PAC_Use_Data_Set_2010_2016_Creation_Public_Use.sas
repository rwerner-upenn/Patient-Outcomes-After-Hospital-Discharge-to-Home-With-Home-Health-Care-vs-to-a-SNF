...
*We require lt instead of le to avoid bundles with two same discharges;
if 0 lt ADMSNDT_ACUTE-ADMSNDT le 89 then gap_5=ADMSNDT_ACUTE-ADMSNDT; else gap_5=.;
if gap_1=. & gap_2=. & gap_3=. & gap_4=. & gap_5=. then delete;
*There should be only 1 nonmissing variable the five variables in each line;
...
proc sort data=temp.merge_all_2010_16_90_pmt;
	by BENE_ID ADMSNDT DSCHRGDT;
run;

*Calculate each of pac and hospital payments within 90 days after hospital discharge;
data temp.merge_all_2010_16_90_pmt_2;
set temp.merge_all_2010_16_90_pmt;
...
*Calculate the sum of pac and hospital payments for each hospital admission;
proc sql;
create table temp.merge_all_2010_16_90_pmt_3 as
	select BENE_ID, ADMSNDT, DSCHRGDT, PMT_AMT, SUM(Pmt_After_Hosp_90) as Pmt_After_Hosp_90_sum, HOSP_PRVDRNUM, DSCHRGDT_90_AFTER, ADMSNDT_90_AFTER,
	...
	from temp.merge_all_2010_16_90_pmt_2
	group by DSCHRG_ID;
quit; *67,591,726;

proc sort data=temp.merge_all_2010_16_90_pmt_3 NODUP;
	by BENE_ID ADMSNDT DSCHRGDT;
run; *67,591,217;

*Merge into the original dataset;
proc sql;
create table Merge_all_1016_90_Pmt as
	...
	from Merge_all_1016 as main
	left join temp.merge_all_2010_16_90_pmt_3 as payment
	on main.BENE_ID=payment.BENE_ID and main.ADMSNDT=payment.ADMSNDT and main.DSCHRGDT=payment.DSCHRGDT;
quit; *69,258,622;

data Pac.Merge_all_1016_90_Pmt;
set Merge_all_1016_90_Pmt;

*In the new BPCI model, 90 day starts after hospital discharge but still includes payment for index hospitalization;
Hosp_Pmt_90=PMT_AMT_PSEUDO;
																					 
if Pmt_After_Hosp_90_sum =. then Total_Pmt_90 = Hosp_Pmt_90;
else Total_Pmt_90=Hosp_Pmt_90+Pmt_After_Hosp_90_sum;
...
proc sql;
create table Merge_all_1016_90_Pmt as 
select a.*, b.Hosp_Pmt_90, b.Pmt_After_Hosp_90_sum, b.Total_Pmt_90, 
            b.Pmt_After_Hosp_90_sum_SNF, b.Pmt_After_Hosp_90_sum_IRF, b.Pmt_After_Hosp_90_sum_HHA, 
            b.Pmt_After_Hosp_90_sum_Other, b.Pmt_After_Hosp_90_sum_Acute
from Merge_all_1016 as a 
left join Pac.Merge_all_1016_90_Pmt as b
